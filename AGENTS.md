...
- **Never edit files in `app/db/drizzle/`** - these are generated by `drizzle-kit` via `bun run db:generate`
- When schema changes are needed, edit `app/db/schemas.ts` only, then let the user run `bun run db:generate`
